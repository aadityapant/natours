        ...
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)(res.data.status, 'The Login was successful'); //To re-route to the home page after 1.5 seconds,
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              // location.reload(true); // This will reload the page from the server, not from the browser cache!
              window.setTimeout(function () {
                location.assign('/');
              }, 1000);
            }
...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiYWRkeWxlYXJucyIsImEiOiJja2Q2cnExbXIwNGdpMnJubmNjOG51YWRvIn0.rvLQDvz6bIbn6cBTGx9opQ';
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ":").concat(loc.description)).addTo(map); // Extend map bounds to include a current location
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? 'http://127.0.0.1:3000/api/v1/users/updatePass' : 'http://127.0.0.1:3000/api/v1/users/updateMe';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            stripe = Stripe('pk_test_51HFfUUDk23fFSNpdJ4pPSBhtg0VyDNvNmF83qIKtNX9BpRE2BgH1Fy3flIwfzpN6174XEUQIcRE1tUAv2n2BzFbB00cijKj2lh');
            _context.next = 4;
...
if (logOut_btn) {
  logOut_btn.addEventListener('click', _login.logout);
}

if (userDataForm) {
  userDataForm.addEventListener('submit', function (event) {
    event.preventDefault(); // const email = document.querySelector('#email').value;
